...
// 市场状态判断函数
function judgeMarketState(klines, adxPeriod = 14, bbPeriod = 20, bbDev = 2, adxThreshold = 25) {
    ...
    const historicalBBWidths = bbResults.slice(-100).map(bb => 
        (bb.upper - bb.lower) / bb.middle
    );
    historicalBBWidths.sort((a, b) => a - b);
    const bbWidthThreshold = historicalBBWidths[Math.floor(historicalBBWidths.length * 0.2)];

    console.log(`当前指标 - ADX: ${currentAdx.toFixed(2)}, BB宽度: ${bbWidth.toFixed(4)}, 阈值: ${bbWidthThreshold.toFixed(4)}`);
    ...
    if (isTrendStrengthHigh && !isVolatilityLow) {
        return { state: "Trending", signal: 1, adx: currentAdx, bbWidth: bbWidth };
    } else if (!isTrendStrengthHigh && isVolatilityLow) {
        return { state: "Ranging", signal: 0, adx: currentAdx, bbWidth: bbWidth };
    } else {
        return { state: "Uncertain", signal: -1, adx: currentAdx, bbWidth: bbWidth };
    }
}
...
class MarketStateMonitor {
    ...
    async analyze() {
        try {
            const currentState = await analyzeMarket(this.symbol, this.interval);
            
            // 检查状态是否发生变化
            if (this.previousState && this.previousState.signal !== currentState.signal) {
